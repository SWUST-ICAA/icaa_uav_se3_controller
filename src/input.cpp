...
void Battery_Data_t::feed(sensor_msgs::BatteryStateConstPtr pMsg)
{
    ...
    if (percentage > 0.05)
    {
        ...
        {
            ROS_INFO("[icaa_se3_ctrl] Voltage=%.3f, percentage=%.3f", volt, percentage);
    ...
    else
    {
        ...
        {
            // ROS_ERROR("[icaa_se3_ctrl] Dangerous! voltage=%.3f, percentage=%.3f", volt, percentage);
            last_print_t = rcv_stamp;
        }
    }
}
...
void Takeoff_Land_Data_t::feed(icaa_uav_se3_controller::TakeoffLandConstPtr pMsg)
{
    msg = *pMsg;
    rcv_stamp = ros::Time::now();
